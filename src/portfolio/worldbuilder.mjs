...
const cards = [
  {
    key: 'boundary',
    note: 'Place the shot. Earn the run.',
    role: 'Simulation · 3D rendering · batting & running controls',
    decision:
      'The ball, fielders and both batters share one fixed-step simulation. A run counts when both runners reach the crease; a six follows the ball’s actual flight.',
    evidence: 'Cricket in a browser. Keyboard, touch and offline play.',
    image: 'built-boundary',
  },
  {
    key: 'bring-something-home',
    note: 'Fast combat. Lasting consequences.',
    role: 'Game design · realm server · client · persistence',
    decision:
      'Predict movement so a dodge feels immediate. Let one realm decide every hit and reward, with atomic writes for what survives the expedition.',
    evidence: 'Cooperative bullet-hell RPG. A complete local build.',
...
function card(item, index) {
  ...
  return `<article class="wb-project wb-project-${p.key}">
    ...
    <div class="wb-project-copy"><div class="wb-project-title"><h3>${esc(p.name)}</h3><span class="wb-live">${p.local ? 'Local build' : 'Live'}</span></div><p class="wb-project-note">${esc(item.note)}</p><p class="wb-project-role">My work / ${esc(item.role)}</p><p class="wb-project-decision">${esc(item.decision)}</p><p class="wb-project-evidence">${esc(item.evidence)}</p><div class="wb-project-links">${caseLink(p.key, 'Inside the build')}${out(p.href, p.local ? 'Play locally' : 'Open project')}</div></div>
  </article>`;
}
...
export default function render() {
  ...
  <section class="wb-cover" aria-labelledby="wb-name" data-scene="murmuration">
    ...
    <header class="wb-nav wb-shell"><a class="wb-brand" href="#main" aria-label="AJ Uppal, home">AJ<span>Engineer & explorer</span></a><nav aria-label="Main navigation"><a href="#work">Work</a><a href="/portfolio/collection/">Collection</a><a href="#about">Meet AJ</a>${email('Contact')}</nav></header>
  ...
  <section id="projects" class="wb-worlds wb-shell" aria-labelledby="wb-worlds-title"><header class="wb-section-heading"><div><p class="wb-kicker">Off the clock / Personal projects</p><h2 id="wb-worlds-title">A few worlds<br><em>of my own.</em></h2></div><p>I learn things by building them. Sometimes that means a shared world; sometimes, finding the right way to move one cube.<span>${homeProjectKeys.length} places to begin. The decisions behind each one.</span></p></header><div class="wb-projects">${cards
    .filter((c) => homeProjectKeys.includes(c.key))
    .sort((a, b) => homeProjectKeys.indexOf(a.key) - homeProjectKeys.indexOf(b.key))
    .map(card)
    .join(
      '',
    )}</div><div class="wb-workbench"><p class="wb-kicker">Still taking shape</p><div>${caseLink('hidamari', 'hidamari')}<span>Ambient light · work in progress</span></div><div>${caseLink('elderwood', 'Elderwood Vale')}<span>Deterministic simulation · work in progress</span></div></div>${discoveries(['slipstream', 'eyeshot', 'beatlayer'])}</section>

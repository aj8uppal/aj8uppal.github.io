...
/* ── One call, generically ────────────────────────────────────────────────
   ...
   customer in it, no vendor, no caller and no product name, and every state is
   named the way the literature names it rather than the way any codebase does.
   It claims nothing about the work that the four points above do not already
   say in plain English.

   And it carries no numbers. Latencies and durations are exactly the kind of
   thing that reads as measurement, and none of these would be: an invented
   millisecond is a lie whether or not anybody checks it. So the trace is
   ordered and unclocked, and the order is the only claim it makes.

   The failure path is the point. The happy path of a voice workflow is four
   lines long and tells a reader nothing about whether the person who built it
   has run one in production. */

export interface TraceStep {
  /** The state, as the system would name it in its own logs. */
  state: string;
  /** True for the states that only exist because something went wrong. */
  recovery?: boolean;
  text: string;
}

export const callTrace: TraceStep[] = [
  {
    state: 'received',
    text: "Call arrives over SIP, from the customer's own telephony rather than the public network.",
  },
  {
  ...
  {
    state: 'tool call',
    text: 'The write is refused. The slot went while the caller was deciding.',
  },
  { state: 'fallback', recovery: true, text: 'No retry helps a slot that is gone. Do not guess.' },
  {
    state: 'transfer',
    recovery: true,
